...
const updateStudent = (req, res) => {
  ...
  db.get(`SELECT u.id FROM users u INNER JOIN roles r ON u.role_id = r.id WHERE u.id = ? AND r.name = 'student'`, [id], (err, student) => {
    if (err) {
...
const deleteStudent = (req, res) => {
  ...
  db.get('SELECT COUNT(*) as count FROM class_enrollments WHERE student_id = ? AND status = "active"', [id], (err, enrollmentCount) => {
    if (err) {
    ...
    if (hard_delete === 'true' && enrollmentCount.count > 0) {
      return res.status(400).json({
        ok: false,
        message: 'No se puede eliminar el estudiante porque tiene matrículas activas. Primero retire al estudiante de todas las clases.'
      });
    }
...
const assignStudentToClass = (req, res) => {
  ...
  db.get('SELECT id FROM classes WHERE id = ?', [classId], (err, classExists) => {
    ...
    db.get(`SELECT u.id FROM users u INNER JOIN roles r ON u.role_id = r.id WHERE u.id = ? AND r.name = 'student'`, [studentId], (err, studentExists) => {
      if (err) {
        console.error('Error al verificar estudiante:', err.message);
        return res.status(500).json({
          ok: false,
      ...
      db.get('SELECT id, status FROM class_enrollments WHERE class_id = ? AND student_id = ?', [classId, studentId], (err, existing) => {
        if (err) {
        ...
        if (existing) {
          if (existing.status === 'active') {
            ...
            db.run('UPDATE class_enrollments SET status = ?, enrolled_at = CURRENT_TIMESTAMP WHERE class_id = ? AND student_id = ?', ['active', classId, studentId], function(err) {
              if (err) {
...
const removeStudentFromClass = (req, res) => {
  ...
  db.get('SELECT status FROM class_enrollments WHERE class_id = ? AND student_id = ?', [classId, studentId], (err, enrollment) => {
    if (err) {
    ...
    if (hard_delete === 'true') {
      db.run('DELETE FROM class_enrollments WHERE class_id = ? AND student_id = ?', [classId, studentId], function(err) {
        if (err) {
      ...
      db.run('UPDATE class_enrollments SET status = ? WHERE class_id = ? AND student_id = ?', ['removed', classId, studentId], function(err) {
        if (err) {
