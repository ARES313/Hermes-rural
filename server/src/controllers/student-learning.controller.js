    ...
    db.all(foldersQuery, [classId], (foldersErr, folders) => {
      ...
      db.all(contentQuery, [classId], (contentErr, rows) => {
        ...
        const validContent = (rows || []).filter((file) => {
          const classSlug = getClassSlug(file.class_name);
          let filePath = path.join(__dirname, '../../storage/clases', classSlug, 'contenido');
          if (file.folder_path) {
            const relativePath = file.folder_path.startsWith('/')
              ? file.folder_path.substring(1)
              : file.folder_path;
          ...
          if (!exists) {
            console.warn(
              `Archivo huérfano omitido (estudiante): ${file.filename} (id: ${file.id})`,
            );
          }
          return exists;
        });
          ...
          db.all(
            ...
            (err4, questions) => {
              ...
              for (const answer of answers) {
                ...
                console.log(
                  `[QUIZ_DEBUG] Q${answer.question_id}: correct="${question.correct_answer}" vs selected="${answer.selected_answer}" => isCorrect=${isCorrect}`,
                );
              ...
              console.log(
                `[QUIZ_DEBUG] RESUMEN: totalQuestions=${totalQuestions}, correctCount=${correctCount}, score=${score}%`,
              );
              ...
              db.run(
                ...
                function (err5) {
                  ...
                  for (const answer of answerRecords) {
                    db.run(
                      `INSERT INTO quiz_answers (submission_id, question_id, selected_answer, is_correct) VALUES (?, ?, ?, ?)`,
