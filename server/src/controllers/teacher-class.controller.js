...
const verifyTeacherOwnership = (classId, teacherId, callback) => {
    db.get('SELECT id FROM classes WHERE id = ? AND teacher_id = ?', [classId, teacherId], (err, row) => {
        if (err) return callback(err);
        if (!row) return callback({ status: 403, message: 'No tienes permiso para acceder a esta clase' });
        callback(null, true);
    });
};
...
const getClassContent = (req, res) => {
    ...
    if (folder_id !== undefined && folder_id !== null && folder_id !== '' && folder_id !== 'null' && folder_id !== 'undefined') {
    ...
    db.get(checkQuery, params, (err, classRow) => {
        ...
        const getFolderInfo = (callback) => {
            ...
            `, [normalizedFolderId, classId], (err, folder) => {
                if (err) {
                    callback(err);
                } else if (folder) {
                    folderInfo = folder;
        ...
        getFolderInfo((err) => {
            if (err) {
                ...
                return res.status(404).json({
                    ok: false,
                    message: err.message === 'Carpeta no encontrada' 
...
const uploadContent = (req, res) => {
    ...
    db.get(checkQuery, params, (err, classRow) => {
        ...
        const processUpload = (folderPathSegments = '') => {
            ...
            db.run(insertQuery, [classId, normalizedFolderId, file.filename, file.originalname, file.mimetype, file.size, userId, description || null], function(err) {
                if (err) {
...
const createQuiz = (req, res) => {
    ...
    verifyTeacherOwnership(classId, userId, (err, isValid) => {
        if (err) {
        ...
        // Crear quiz
        db.run('INSERT INTO quizzes (class_id, teacher_id, title, description, status) VALUES (?, ?, ?, ?, ?)',
            ...
            function(err) {
                ...
                const insertQuestion = (question, index) => {
                    db.run(`INSERT INTO quiz_questions 
                        ...
                        [quizId, question.question_text, question.option_a, question.option_b,
                         question.option_c, question.option_d, question.correct_answer, index],
                        (err) => {
                            if (err) {
...
const getQuizById = (req, res) => {
    ...
    db.get('SELECT * FROM quizzes WHERE id = ?', [quizId], (err, quiz) => {
        ...
        db.all('SELECT * FROM quiz_questions WHERE quiz_id = ? ORDER BY order_index ASC', [quizId], (err, questions) => {
            if (err) {
...
const updateQuizStatus = (req, res) => {
    ...
    db.get('SELECT q.*, c.teacher_id FROM quizzes q INNER JOIN classes c ON q.class_id = c.id WHERE q.id = ?', [quizId], (err, quiz) => {
        if (err) {
...
const createTask = (req, res) => {
    ...
    verifyTeacherOwnership(classId, userId, (err, isValid) => {
        ...
        db.run(`INSERT INTO tasks (class_id, teacher_id, title, description, due_date, status)
                VALUES (?, ?, ?, ?, ?, 'active')`,
...
const updateTask = (req, res) => {
    ...
    db.get('SELECT t.*, c.teacher_id FROM tasks t INNER JOIN classes c ON t.class_id = c.id WHERE t.id = ?', [taskId], (err, task) => {
        if (err) {
...
const getTaskSubmissions = (req, res) => {
    ...
    db.get(`SELECT t.*, c.teacher_id FROM tasks t 
...
const viewTaskSubmission = (req, res) => {
    ...
    db.get(query, [submissionId], (err, submission) => {
        ...
        if (!fs.existsSync(filePath)) {
            return res.status(404).json({ ok: false, message: 'Archivo no existe en el servidor', filePath });
        }
...
const gradeSubmission = (req, res) => {
    ...
    db.get(`SELECT ts.*, t.class_id, c.teacher_id 
            FROM task_submissions ts
            INNER JOIN tasks t ON ts.task_id = t.id
            INNER JOIN classes c ON t.class_id = c.id
            WHERE ts.id = ?`, [submissionId], (err, submission) => {
        if (err) {
        ...
        db.run(`UPDATE task_submissions SET grade = ?, feedback = ?, graded_at = CURRENT_TIMESTAMP WHERE id = ?`,
...
const enrollStudentToMyClass = (req, res) => {
    ...
    db.get('SELECT id FROM classes WHERE id = ? AND teacher_id = ?', [classId, teacherId], (err, classRow) => {
        if (err || !classRow) {
            return res.status(403).json({ ok: false, message: 'No tienes permiso sobre esta clase' });
        }

        db.get('SELECT id FROM class_enrollments WHERE class_id = ? AND student_id = ?', [classId, studentId], (err, existing) => {
            if (existing) {
                return res.status(400).json({ ok: false, message: 'El estudiante ya está matriculado' });
            }

            db.run(`INSERT INTO class_enrollments (class_id, student_id, status) VALUES (?, ?, 'active')`,
                [classId, studentId], function(err) {
                    if (err) return res.status(500).json({ ok: false, message: 'Error al matricular estudiante' });
...
const getMyClassStudents = (req, res) => {
    ...
    db.get('SELECT id FROM classes WHERE id = ? AND teacher_id = ?', [classId, teacherId], (err, classRow) => {
        ...
        `;
        db.all(query, [classId], (err, rows) => {
            if (err) return res.status(500).json({ ok: false, message: 'Error al obtener estudiantes' });
        ...
        db.get(fileQuery, [fileId, classId], (err, file) => {
            ...
            verifyTargetFolder((err, targetFolder) => {
                ...
                if (targetFolderId !== null && targetFolder) {
                    targetFolderPath = targetFolder.path.startsWith('/') 
                        ? targetFolder.path.substring(1) 
                        : targetFolder.path;
                    targetPath = path.join(targetPath, targetFolderPath);
...
// ==================== VER Y DESCARGAR CONTENIDO ====================
    ...
    const viewContentFile = (req, res) => {
        ...
        db.get(query, [fileId, classId], (err, content) => {
            ...
            if (userRole === 'student') {
                db.get('SELECT id FROM class_enrollments WHERE class_id = ? AND student_id = ? AND status = "active"', [classId, userId], (err, enrollment) => {
                    if (err || !enrollment) {
            ...
            function sendFile() {
                ...
                if (content.folder_path) {
                    const relativePath = content.folder_path.startsWith('/') ? content.folder_path.substring(1) : content.folder_path;
                    filePath = path.join(filePath, relativePath, content.filename);
                } else {
                    filePath = path.join(filePath, content.filename);
                }
