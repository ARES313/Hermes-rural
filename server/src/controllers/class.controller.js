...
const createClass = (req, res) => {
  ...
  db.run(
    query,
    [name, description || null, teacher_id, schoolYearValue, statusValue],
    function (err) {
      if (err) {
      ...
      db.get(
        `SELECT c.*, u.full_name as teacher_name FROM classes c INNER JOIN users u ON c.teacher_id = u.id WHERE c.id = ?`,
        [this.lastID],
        (err, row) => {
          if (err) {
...
const deleteClass = (req, res) => {
  ...
  db.get('SELECT * FROM classes WHERE id = ?', [id], (err, classExists) => {
    ...
    if (hard_delete === 'true') {
      ...
      db.run(
        'UPDATE classes SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
        ['inactive', id],
        function (err) {
          if (err) {
