          ...
          function (err) {
            ...
            if (usersProcessed === users.length) {
              ...
              db.get(`SELECT id FROM users WHERE email = 'docente@redes.local'`, (err, row) => {
                ...
                // Obtener student ID
                db.get(
                  `SELECT id FROM users WHERE email = 'estudiante@redes.local'`,
                  (err, row) => {
                    if (err) {
                    ...
                    if (!teacherId || !studentId) {
                      console.error(
                        '❌ No se pudieron obtener los IDs del docente o estudiante. Abortando.',
                      );
                      db.close();
                      return;
                    }
                    ...
                    db.run(
                      ...
                      function (err) {
                        ...
                        // Obtener class ID
                        db.get(
                          `SELECT id FROM classes WHERE name = 'Redes 10A' AND teacher_id = ?`,
                          [teacherId],
                          (err, row) => {
                            if (err) {
